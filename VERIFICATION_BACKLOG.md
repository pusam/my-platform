...
- **④ 시그널 평가 방법론 편향(측정 인프라)**: hit 에 거래비용 0.36% 미반영 · 진입가=기록 시점 장중가(체결
  ...
  - **✅ 비교 창 정렬 + 고유일수 게이트 (2026-08-21)**: 측정 파이프라인 감사에서 **대조군 비교가 서로 다른
    기간을 비교하고 있었음**을 확인(P0). `getAccuracyByBand` 가 phase-38 컷오프(6/25)부터 조회한 결과를
    그대로 넘겨, 시그널은 6/25~(40거래일)·대조군은 8/5~(도입일, 12거래일)로 집계됐다 → edge 가 "점수의
    기여"가 아니라 **기간(시장) 차이**를 포함. 6~7월 창 대부분이 BEAR 라 점수가 실제보다 나빠 보이는 방향.
    → `aggregateControlComparison` 이 **양쪽 모두 평가 완료 행이 있는 날짜의 교집합**으로 두 집합을 함께
    좁히도록 수정, 응답에 `comparisonFrom/To/Days` 노출. 밴드/카테고리 집계는 전체 창 유지(여기서만 좁힘).
    함께 **고유일수 게이트** 추가 — 보드 시그널은 하루 최대 10건이 뭉치고 대조군도 1:1로 붙어 행 수 30건이
    **3거래일치**일 수 있다. 유효 자유도 과대 → SE 과소 → |z| 과대 → 우연을 "유의"로 선언하던 것을
    `MIN_CONTROL_DISTINCT_DAYS=10` 으로 차단(P3-11 이 지수 축에 적용한 원리를 대조군 비교에도 적용 —
    이 축에 누락돼 있었다). 테스트 `ControlGroupTest` 3건 추가(창 정렬·교집합 없음·하루 뭉침).
    ⚠ **정렬을 제거하면 대조군을 만든 목적이 무효가 된다.**
...
## AUDIT 2026-08-21. 데이터 입력층 + 측정 파이프라인 감사 — 수정 6건 / 잔여 7건

> 지금까지 감사는 채점 로직(downstream)만 봤다. 이번엔 **입력 데이터**와 **측정 자체**를 봤고
> 양쪽 모두에서 구조적 결함이 나왔다. 핵심 교훈: **측정이 정직해도 입력이 오염되면 소용없다.**

### ✅ 같은 날 수정 (2026-08-21)
1. **대조군 비교 창 정렬 + 고유일수 게이트** (P0) — P2-19 ④ 항목에 상세 기록.
2. **휴장일 달력 보강** — `MarketCalendarService` 에 대체공휴일 테이블(2025~2027)·근로자의날(5/1)·
   연말폐장(12/31) 추가. 미수록 휴장일 다음 거래일 오전 수급·기술 노후가드가 유령 거래일을 물고
   정상 데이터를 노후로 오판 → 동시 미채점 → 모닝브리핑·오늘 후보 전멸이던 결함(연 5~6회 확정 발생,
   **2026-08-17 광복절 대체휴일이 실제로 미수록 상태로 지나감**). ⚠ 임시공휴일(정부 수시 지정)은
   여전히 수동 추가. 매년 갱신은 음력 테이블과 같은 주기로.
3. **거래정지 게이트 KRX 호출 OTP 2단계 전환** — `StockStatusService` 가 단발 getJsonData(Referer 만)
   死패턴(SHORT_SELLING_DEAD_FEED_DIAGNOSIS §2 실측: 본문 "LOGOUT" 구조적 거부)이라
   `activeStockCodes` 영구 빈 집합 = **isActive/filterActiveStocks 전면 fail-open**(거래정지·상폐가
   추천·발굴·봇 유니버스에 무필터 유입) 상태였을 가능성이 높다. `MarketTimingService.getKrxOtp` 의
   살아있는 2단계 패턴으로 전환 + trdDd=직전 거래일(08:30 엔 당일 시세 없음. 신규 상장 D0 하루 사각은
   인지된 트레이드오프) + 파싱 순수함수 분리(`parseStockCodes`, LOGOUT/HTML→빈 집합, 테스트 4건) +
   **동기화 실패 지속 시 일 1회 리스크 채널 경보**(이전엔 log.error 한 줄이라 몇 주를 조용히 지나감).
   배포 후 확인: `[종목상태] KRX 동기화 완료 — 총 N건`(정상 ≈2,700+).
4. **주간 리포트 주 선택 수정** — 일요일 크론이 "오늘 끝나는 주"를 집계해 평가 완료된 **월·화 시그널만**
   담기고(수~금은 다음 주 평가) 요일 구성 차이가 "예측력 악화" 경고로 오독되던 결함 →
   항상 직전 완료 주(그 주 금요일 시그널까지 평가 완료)로. **이 수정 이전 주차 행은 월·화 표본으로
   확정돼 있다 — 주간 추세 해석 시 인지**(수동 트리거 재실행 시 UPSERT 자가 보정).
5. **같은 종목·같은 날 이중 계상 dedup** — V36 UNIQUE 가 (type, code, date) 라 장중 BUY→STRONG_BUY
   승격 시 2행. 같은 3일 수익률 창을 공유하는 비독립 관측이 65~74/75~84 밴드에 동시 계상(75 경계
   수급의존 종목군 이중 가중) → `dedupPerStockDay` 로 밴드·주간·대조군 비교 공통 적용.
   record 경로·V36 UNIQUE 는 불변(기록은 그대로, 집계에서만 대표 1건).
   **대표 선택은 같은 날 2차 리뷰(A-1)로 "마지막"→"최초 기록"으로 전환** — 마지막 대표는 장중
   STRONG_BUY→BUY 강등 행(이후 수익률 나쁠 부류)이 상위 밴드에서 체계적으로 제거되는 생존편향.
   최초 기록 = 사용자가 실제 행동 가능했던 시점의 등급·가격(look-ahead 없음).
6. **소수정 2건** — `/api/signal-outcomes/accuracy` avg null NPE 가드(터지면 엔드포인트 전체 500) +
   record 스킵 가시화(컷 통과 종목이 시세 캐시 콜드로 조용히 미기록되면 표본이 "평온한 날"로 편향 —
   warn 카운트 추가).
...
### ⛔ R1 진단 정정 (2026-08-26 오후, prod 실측)

**위 "R1 처리" 절의 전제가 틀렸다.** 배포 후 운영 DB 를 실제로 조회해서 확인했다.

#### 틀린 것

"수집기가 매일 `reportDate=today` 행을 쌓고 `findLatestTwoQuartersPerStock` 이 오늘/어제를 비교한다"
→ **그런 일은 일어난 적이 없다.** 그 쿼리는 `WHERE net_income IS NOT NULL` 로 거르는데,
일별 스냅샷 행에는 `net_income` 이 **아예 없다**(NULL). 그래서 쿼리가 보는 것은
네이버 크롤이 넣은 **진짜 분기 행**뿐이다.

두 최신 행의 간격 분포(prod 실측, 431종목):

| gap_days | 종목 | 120일 가드 |
|---|---|---|
| 0 | 1 | 탈락 |
| 90 · 91 | **90** | ✅ 통과 = 진짜 인접 분기 |
| 184 | 203 | 탈락 (한 분기 결측) |
| 275 | 136 | 탈락 (두 분기 결측) |
| 365 | 1 | 탈락 |

전부 분기 배수다. `legacyTotal 60 / legacyScoring 38` 이 통과 가능 ~90종목에서 나온 것과 맞는다.

#### 실제 결함 — 더 크고 다른 것

**KIS 손익계산서(FHKST66430300)와 재무상태표(FHKST66430400) 호출이 조용히 죽어 있다.**
2026-08-25 일별 행 434건 기준:

```
per / pbr / roe                     434 / 434   ✅ 재무비율 API(FHKST66430200) 정상
revenue / operating_profit /
net_income / total_equity             0 / 434   ❌ 손익계산서·재무상태표 전멸
```

파급:
- **earnings 축**: 네이버 크롤 분기 행에만 의존 → 커버리지 **434종목 중 채점 가능 ~90**
- **composite `scoreValueStability`**: `operatingProfit`·`totalEquity` 를 최근 10행에서 찾는데
  그 10행이 전부 일별 행이라 NULL → 가치 축 일부가 조용히 빠진 채 채점돼 왔을 수 있다
- **V55 파이프라인(어제 배포)**: `persistQuarterlyRows` 가 **바로 그 죽은 응답**을 담는다
  → 현 상태로는 15:38 배치가 돌아도 **0건**. 플래그를 켤 수 없다.

**원인 미확정.** 실패 경로는 이미 WARN 으로 찍히므로(`[손익계산서] … API 오류: {msg1}` /
`output이 비어있음` / `API 응답 없음`) **다음 배치(08:30·15:38) 로그가 원인을 바로 말해준다.**
추측으로 고치지 말 것 — msg1 을 읽고 판단한다.

#### 부수 확인

- **유니버스가 434종목뿐**(KOSPI+KOSDAQ ~2,700). R5 자기참조가 만든 상태이고, 같은 날 고친
  합집합이 다음 배치부터 푼다. ⚠ 종목당 300ms sleep 이라 **수집 시간이 6배**로 늘어난다 —
  첫 배치 소요시간을 확인할 것.
- `report_date` 최댓값이 **2026-12-31(미래)** — 342행. 코드 주석의 "미래 일자 12-31 annual row".
  120일 가드가 막아 earnings 엔 무해하지만 데이터 위생 문제로 남는다.

#### 교훈

**코드만 읽고 데이터를 안 봤다.** `setReportDate(LocalDate.now())` 한 줄을 보고 "테이블 전체가
일별 스냅샷"이라 단정했는데, 같은 테이블에 **다른 writer(네이버 크롤)가 분기 날짜로 쓰고 있었다**.
백로그가 "행 종류가 섞여 있다"고 적어둔 게 그 뜻이었는데 반대로 읽었다.
`[[verify-formulas-with-real-data]]` 가 말하는 그대로 — **운영 DB 한 번 조회했으면 30초에 끝났을 일.**

#### V55 작업은 버리지 않는다

설계는 여전히 맞다(KIS 는 종목당 4분기를 한 번에 주므로 결측 분기를 메우고 커버리지를 넓힌다).
다만 **목적이 "죽은 축 살리기"가 아니라 "얇은 축 두껍게 하기"로 바뀌었고**, 그 전에
**손익계산서 API 부활이 선행 조건**이다. 그게 안 되면 V55 는 영원히 빈 테이블이다.
...
> fail-open 가능성을 안고 있다.

### 2차 리뷰 (2026-08-21 같은 날) — 최근 커밋 적대적 리뷰 + 매수 직전 판단 계층 감사
> 이날 오전 수정분 자체를 적대적으로 재검증(자기 코드 리뷰)했고, 아직 전용 감사가 없던
> "매수 버튼 직전 화면"(결론카드·체크리스트·보드·🎯종합·브리핑)을 처음 감사했다.
> **오전 수정에서도 P1 4건이 나왔다** — 수정 직후 리뷰의 가치 실증.

**✅ 2차에서 바로 수정 (같은 날)**
- dedup 대표를 **최초 기록**으로 전환 + 동시각 id tie-break(A-1/A-8, 위 5번 항목 참조)
- 주간 리포트 주 선택을 `resolveTargetWeekEnd` 순수함수로 — **수동 트리거(평일)** 도 온전한 주만
  집계(A-9: 크론만 고쳐선 월요일 수동 실행이 월·화 표본 스냅샷을 UPSERT 영속화) + prior 스트릭
  필터 `isBefore`(A-10: 미래 주 스냅샷 혼입)
- 대조군 비교에 `excludedSignalDays` 노출(A-4: 짝 없는 날 제외가 조용히 일어나던 것 관측 가능화)
- **보드 "이력 90일 실측" 열 오염 격리**(F15, 판단계층 감사 최우선): `aggregateTrackRecordByCodes` 에
  types 필터 — 이전엔 그 종목이 무작위로 뽑힌 **대조군(CONTROL_RANDOM) 행**·타 엔진(AI/COMPOSITE/
  SURGE) 시그널까지 "N회 중 M회 적중"으로 합산·정렬됐다. accuracy/timeseries/compare 쿼리에서도
  CONTROL_RANDOM 제외(P3-K)
- 추천 스냅샷 크론에 **휴장일 가드**(가드만, cron 시각 불변) — 8/17 대체휴일에 유령 시그널·대조군
  행이 실제로 기록됐다(A-5 원인 차단. 이미 쌓인 8/17 행 정리는 서버 복구 후 R12)
- `StockStatusService` **시장별 부분 실패 게이트**(B-5: KOSPI 성공+KOSDAQ 실패면 합산 950>100 으로
  통과해 KOSDAQ 1,700종목 무음 fail-CLOSED — 발굴·봇·대조군 유니버스에서 소멸) + **경보 조건을
  lastSyncTime 48h 노후로**(B-6: isEmpty 경보는 "성공 후 사망" 최빈 시나리오에서 영영 안 울림)
- **오늘후보 워밍 원복**(C-3: 08:00 getTop5 = 전일 스냅샷이라 07:30 브리핑 워밍과 동일 집합 —
  실익 0 + 발굴 몫 잠식. CLAUDE.md §4b 정정)
- **공시 섹션 종목 전환 혼합 상태 차단**(C-1 P1: stockCode 변경 직후 부모 stockName 은 이전 종목
  값 — 코드 매핑 실패 시 이름 폴백이 **남의 회사 공시**를 정상 데이터로 표시. 코드 변경 시 이름
  없이 조회, 새 이름 도착 시 '확인 불가'만 재조회)

**⚠ 2차 잔여 (R8~, 착수 조건 포함)**
| # | 항목 | 심각도 |
|---|---|---|
| R8 | `getAccuracy`(타입별, 결론카드 상단 줄)에 dedup·phase-38 컷오프 미적용 — 같은 카드의 밴드표와 다른 기준(A-3). SQL 집계라 행 로드 전환 필요 | P2 |
| R9 | dedup 후 승격일엔 대조군이 시그널보다 1행 많은 잔여 비대칭(A-2) — 대조군 기록을 (code,date) 1회로 제한하는 record 측 수정 검토 | P2 |
| R10 | 보드 이력 열 SQL 이라 승격일 2행이 2회 계상 잔여 | P3 |
| R11 | 오늘 후보 재료 배지 공백 — 11:30 첫 스냅샷 이후 워밍 크론 신설 판단(quota·시각) | P2 |
| R12 | 8/17 유령 행(signal_outcome·investor_trade·financial 등) 정리 — 서버 DB 필요 | P2 |
| R13 | **결론카드 입력 노후 가드 부재**(F1): 최대 30일 전 스냅샷의 STRONG_BUY 가 "매수 적기" + 오늘 가격 매매계획으로 렌더. F2(노후 표시 분 단위)와 함께 | **P1** |
| R14 | **결론 룰 3 이 총점 조건 없이 수급≥15·기술<8 을 BUY 승격**(F4) — §4 "BUY 55~74" 위반 + P1-6 역상관 축. 산식(판정 룰) 변경이라 설계 검토 선행 | **P1** |
| R15 | **체크리스트 노후·fail-open 정직화**(F10/F11/F12/F13): 연속매수 endDate 노후 가드(봇 5일 규칙 이식)·공매도 기준일 검증+표시·tradable fail-open 을 "미확인"으로·분모 극성 통일 | **P1** |
| R16 | 보드 ① 점수 as-of 미표시(F16 — 오늘 탭엔 있는 `dataTime` 을 보드 DTO 에도) + 🎯종합 소스 설명·as-of(F19) | P2 |
| R17 | 복합신호 evaluator 소스 장애가 "미매칭"으로 위장(F20) — `available` 구분 필드(보드 timingAvailable 패턴 이식). 체크리스트 4번에 전파됨 | P2 |
| R18 | 브리핑 기준일 미표시(F21/F22 — DTO 에 이미 있는 analysisDate/dataAge/endDate 를 본문에) + 관심종목 "현재가"=전일종가 라벨(F24) | P2 |
| R19 | 결론카드 MFE/MAE SQL 집계 dedup 미적용(F6) + basePrice 지연 고지(F7) + 수급 verdict 색 모순(F3: 카드 ≥15 초록 vs 보드 ≥10 역상관 경고) | P2 |
| R20 | 체크리스트가 봇 룰 부분 복제(F14: 봇 7게이트 중 1개만 노출 + 임계 별도 상수 drift) — "봇 룰 노출" 문구와 실제 불일치 | P2 |
| R21 | 4화면(카드/보드/체크리스트/브리핑) 시점 불일치 미고지(F26) — R13/R16/R18 해소 후 잔여 재평가 | P2 |
| R22 | 달력 보강의 소급 효과 고지(B-1: 수급 스트릭이 휴장일 걸침으로 변동 — 옳은 방향이나 산식 입력 변동임을 인지) / 5/1·12/31 DART·재무 수집 스킵 여부 확인(B-2) / 2028 미수록 연도 무경고 열화(B-4) / MDCSTAT01501 에 거래정지 종목 포함 여부 실측(B-7 — 사실이면 "거래정지 게이트"가 아니라 "상폐 게이트") | P2~P3 |
| R23 | 체크리스트 열람이 COMPOSITE 시그널을 record → 열어본 종목 선택 편향 표본(F27) | P3 |
