...
- **주식 정보**
  - **시장 지표**: 52주 신고가/신저가, 시가총액 순위, 거래대금 순위, 급등/급락주
    - 실시간 순위 TOP 50
    - 52주 최고가/최저가 대비율 자동 계산
    - 시가총액, 거래대금 상위 종목
    - PER/PBR 데이터 포함
...
- **금융 정보**
  - 금/은 시세 조회 및 자산 관리
  - 가계부 수입/지출 관리
  - **투자자별 매매 동향** (한국투자증권 API)
    - 외국인/기관/개인 투자자별 거래 상위 종목 조회
    - 종목별 투자자 매매 이력 확인
    - 실시간 주식 시세 연동
...
한국투자증권 API는 다음과 같은 제한사항이 있습니다:

1. **거래일 데이터만 제공**: 장이 열린 날(평일 장 개장일)의 데이터만 조회 가능
   - ❌ 주말 (토요일, 일요일)
   - ❌ 공휴일
   - ❌ 장 마감 전 당일 데이터 (장 마감 후 익일부터 조회 가능)

2. **데이터 갱신 시점**: 일반적으로 장 마감 후 수 시간 내에 데이터가 업데이트됨

3. **현재 상황** (2026년 1월 22일 기준):
   - 1월 19일(일): 주말 - 데이터 없음
   - 1월 20일(월): 설날 연휴 또는 휴장일 - 데이터 없음  
   - 1월 21일(화): 당일 데이터는 장 마감 후 익일부터 조회 가능

### 데이터 수집 방법

1. **대시보드에서 수집**:
   - 유저 대시보드 → "투자자 매매 동향" 위젯 클릭
   - "데이터 수집하기" 버튼 클릭
   - 최근 5일치 데이터 자동 수집 (거래일만)

2. **API 직접 호출**:
   ```bash
   # 특정 일자 데이터 수집
   POST /api/investor/collect?date=2026-01-17
   
   # 최근 N일 데이터 수집
   POST /api/investor/collect/recent?days=5
   ```

3. **데이터 조회**:
   - 대시보드 위젯에서 "투자자 매매 동향" 클릭
   - 매수/매도 탭 선택
   - 외국인/기관/개인 탭 선택
   - 종목 클릭 시 상세 매매 이력 확인
...
- **빈 데이터 반환**: 해당 일자가 휴장일이거나 데이터가 아직 갱신되지 않은 경우
- **해결 방법**: 이전 거래일의 데이터로 수집 시도 (예: 1월 17일 금요일)
