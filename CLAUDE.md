...
- **재무 테이블 두 개는 역할이 다르다 — 섞지 말 것(2026-08-26, R1)**: `stock_financial_data` 는 이름과 달리 **일별 스냅샷**이다(매일 `reportDate=오늘` 로 TTM 합 + **당일 주가 기반** PER/PBR/ROE 한 행). `stock_quarterly_financial`(V55)이 **분기 원본**이다(KIS `stac_yymm` 단위, 주가 무관). **"최신 2행 = 최근 2분기"로 읽지 말 것** — 일별 테이블에서 그렇게 읽으면 "오늘 vs 어제"라 변화율 ≈ 0 이고, 그게 earnings 카테고리가 死였던 원인이다(인접분기 120일 가드는 gap=1 이라 통과, `isEarningsReportFresh(200일)` 도 reportDate 가 늘 오늘이라 no-op). 분기 원본은 **이미 매일 받던 응답**을 적재만 한 것이라 새 API 호출·TTM 산식 변경이 없다(PER/PBR 무영향). 분기 판정 순수함수는 `QuarterlyFinancials`(누적 YTD 환산·인접분기 3개월·비교쌍 선택), 분류 산식은 두 경로가 `EarningSurpriseService.classify` **단일 출처**로 공유한다. ⚠ 결측 파싱 함수가 둘인 것도 의도 — `parseBigDecimal`(결측→0, TTM 합산용) vs `parseBigDecimalOrNull`(결측→null, 분기 원본용). 분기 원본에 0 을 넣으면 "영업이익 0억"이라는 거짓 사실이 남아 다음 분기 비교에서 유령 변화율이 된다. **전환은 `recommendation.earnings.quarterly-source`(기본 false)** — 켜면 composite 총점·validCount·후보 수가 동시에 움직이므로 켠 날짜가 측정 표본 경계다(`docs/SCHEDULE_DECISIONS.md` 에 기록). 비교는 **QoQ** 이고 YoY 전환은 별도 판정 안건.
