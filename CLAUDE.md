...
- **데이터 이상 점검은 규칙이 하고 크루는 읽기만 한다(2026-08-27)**: 크루는 툴이 없어 스스로 데이터를 못 뒤진다(§7, 의도된 설계) — 그래서 **탐지는 결정적 규칙**(`DataAnomalyRules`)이 하고 크루는 결과를 받아 **우선순위·해석**만 한다. "AI 에게 이상한 데이터를 찾아오라"는 시킬 수 없고 시켜서도 안 된다. **규칙은 전부 실제로 겪은 사고에서 나온다** — 누적 판정 쏠림(8/27 96% 오판) · 미래 날짜 행(8/26 342행) · 분기 노후 · 수집 정체. **불변식**: ① 정상이면 null 을 돌려 **조용하다**(정상에서 시끄러우면 사람이 무시하게 되고 다음 사고도 지나간다) ② 각 규칙은 "무엇을 확인하라"까지 `detail` 에 담는다 ③ 점검 실패는 `dataAvailable=false` — '이상 없음'과 구분(§4c) ④ 크루 컨텍스트에선 KPI·캘린더와 함께 **head 에 들어가 잘리지 않는다**(잘리는 것은 FLAGGED 뿐). 회귀는 `DataAnomalyRulesTest` — 그날 실측 수치로 "이 규칙이 있었다면 울렸을까"를 고정한다.
