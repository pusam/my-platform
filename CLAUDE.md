...
- 모닝브리핑(07:30) 후행 워밍: BUY 컷(55) 이상 **상한 5종목** — Gemini quota 가드. 근거 없이 늘리지 말 것. **일괄 워밍(08:00 `CatalystWarmingService`, P2-CAT3) 대상 = 관심 > 보유 > 오늘 매수후보(getTop5 BUY컷55 상위5, 2026-08-20 추가) > 발굴 5트랙, 전체 40 컷** — 오늘 탭/보드/이력은 일캐시 **read-only**(리스트 화면에서 classify 트리거 금지)라 워밍이 배지 공백을 채운다(07:30 워밍은 전일 스냅샷 기준이라 08:00 재계산 후 신규 후보를 못 덮음 → 오늘후보 그룹이 그 공백 담당).
...
- **시그널 측정 위생(2026-08-21) — 되돌리지 말 것**: ① 같은 종목·같은 날 BUY→STRONG_BUY 승격 2행은 집계에서 **마지막 기록 1건으로 dedup**(`SignalOutcomeService.dedupPerStockDay` — 밴드·주간·대조군 비교 공통. record/V36 UNIQUE 는 불변, 집계만). ② 대조군 비교(`aggregateControlComparison`)는 **양쪽 공통 날짜 교집합으로 창 정렬** + 행수 30 과 별개로 **고유일수 ≥10**(`MIN_CONTROL_DISTINCT_DAYS`) — 정렬을 빼면 edge 가 기간(시장) 차이를 담고, 고유일수 게이트를 빼면 3거래일 뭉친 표본으로 "유의" 선언이 된다. ③ 주간 리포트(`SignalWeeklyReportService`)는 **직전 완료 주**(오늘 이전 마지막 일요일 종료 주)를 집계 — "오늘 끝나는 주"로 되돌리면 3거래일 평가 지연 때문에 월·화 시그널만 담긴다.
