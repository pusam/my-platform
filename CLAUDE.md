...
- **리스크 공시 페널티는 composite 실감점**(2026-07-28): `StockScore.riskPenalty=5` 를 수급 캡과 동일한 **raw 합산 3지점(필터/toDto/getNormalizedTotal)에서만 차감**(카테고리 표시값·validCount 불변, `RecommendationRiskPenaltyTest`). 이전 valueStability −5 만으론 composite 랭킹에 실효 0(no-op)이었음. 5트랙은 종전대로 score −5. **거래정지/상폐는 감점이 아니라 제외** — `StockStatusService.isActive` 게이트가 composite+5트랙 전부에 적용(동기화 전 fail-open 은 기존 semantics). **KRX 동기화는 OTP 2단계 필수(2026-08-21)** — 단발 `getJsonData`(Referer 만)는 세션 거부("LOGOUT") 死패턴이라 단발 방식으로 되돌리면 게이트가 조용히 전면 fail-open 된다. 동기화 실패 지속 시 일 1회 리스크 경보(침묵 금지) 유지할 것.
