...
- **차트기법 신호 (2026-06-29)**: 펨코 추세추종 기법을 별도 모듈로 추가 — `/api/v2/chart/timing`(정배열·이격도·엔벨로프 눌림목·박스 타이밍)·`/api/v2/chart/sector-strength`(섹터 상대강도). 지표는 `app/indicators/*` 순수함수(pytest 有). ⚠ **산식 미검증(검증 전 베타)** — 노출 위치(확정): **타이밍 = '오늘' 탭 매수후보 아래 '검증 전 베타' 별도 섹션**(`TodayBriefingTab.vue`, momentum 55컷 후보와 분리·대체 아님), **섹터강도 = '발굴' 탭 상단 상시 배지**(`StockTradingDashboardV2.vue`). 봇/종합추천/매수후보 랭킹 편입 금지(검증 = VERIFICATION_BACKLOG P2-12). Java 소비 = `ChartPatternClient`(best-effort) → `ChartSignalController`(`/api/recommendation/trend-pullback-top10`·`/sector-strength`, 응답 `unverified=true`).
...
- **차트 기법 통합 시 발굴/매수후보 스코어러는 항상 분리한다(2026-06-29).** 추세추종 기법은 성격이 다른 두 신호로 쪼개진다: ① 섹터 상대강도='덜 빠지는 섹터' = **발굴 탭 상단 배지**(유니버스 필터), ② 정배열+눌림목 진입 타이밍(momentum 아님, 추세 안의 mean-reversion, momentum 과 objective 정반대) = **'오늘' 탭 매수후보 아래 '검증 전 베타' 별도 섹션**. **기존 momentum 스코어러(`RecommendationService`/`getTop5`)에 욱여넣지 말고 별도 모듈**(python `app/indicators/*` + Java `ChartPatternClient`/`ChartSignalRanker`/`ChartSignalController`)로 둘 것. 타이밍은 momentum 매수후보를 **대체하지 않고 별도 베타 리스트로 병렬 노출**. 검증(P2-12) 전엔 실거래/봇 신호로 승격 금지(`unverified=true`). 참고: momentum(`getTop5`)은 이미 '오늘' 탭 전용이라 발굴↔매매 공유 잔재 없음.
