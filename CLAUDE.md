...
- **🔴 KIS 손익계산서·재무상태표 호출이 죽어 있다(2026-08-26 실측, 원인 미확정)**: 일별 행 434건 중 `per/pbr/roe` 는 434/434 채워지는데(FHKST66430200 정상) `revenue/operating_profit/net_income/total_equity` 는 **0/434**(FHKST66430300·FHKST66430400 전멸). earnings 커버리지와 composite `scoreValueStability`(영업이익·자본총계)를 동시에 먹는다. 실패 경로는 이미 WARN 으로 찍히므로(`[손익계산서] … API 오류: {msg1}` / `output이 비어있음` / `API 응답 없음`) **배치 로그의 msg1 을 읽고 고칠 것 — 추측 수정 금지.** `stock_quarterly_financial`(V55) 적재도 이 응답에 물려 있어 **이게 살아나기 전엔 빈 테이블**이다.
