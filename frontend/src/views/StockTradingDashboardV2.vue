...
function getFallbackAiStrategy() {
  return {
    strategies: {
      SCALPING: [
        { stockCode: '005930', stockName: '삼성전자', currentPrice: 83400, changeRate: 2.45, score: 82, aiScore: 88, aiComment: '반도체 수급 쏠림, 외인 매집 가속', aiThemes: 'AI반도체,수급우량,HBM', originalScore: 78, rankNum: 1, reason: '거래량 450% 급증', volumeRatio: 450 },
      ...
      SWING: [
        { stockCode: '005380', stockName: '현대차', currentPrice: 248000, changeRate: 1.22, score: 85, aiScore: 90, aiComment: '글로벌 EV 판매 1위 질주', aiThemes: '전기차,글로벌수출,저PER', originalScore: 81, rankNum: 1, reason: 'ROE 18.2%, PER 5.8배', per: 5.8, roe: 18.2, operatingMargin: 10.5 },
      ...
      TURNAROUND: [
        { stockCode: '003670', stockName: '포스코퓨처엠', currentPrice: 264500, changeRate: 2.33, score: 80, aiScore: 84, aiComment: '양극재 수주 급증, 흑자전환 확정', aiThemes: '실적턴어라운드,2차전지', originalScore: 77, rankNum: 1, reason: '적자→흑자 전환', turnaroundType: 'LOSS_TO_PROFIT', netIncomeChangeRate: 999.99 },
        { stockCode: '247540', stockName: '에코프로비엠', currentPrice: 156000, changeRate: 1.85, score: 76, aiScore: 80, aiComment: '하이니켈 양극재 흑자 전환', aiThemes: '흑자전환,2차전지소재', originalScore: 73, rankNum: 2, reason: '적자→흑자 전환', turnaroundType: 'LOSS_TO_PROFIT', netIncomeChangeRate: 999.99 },
      ...
      VALUE: [
        { stockCode: '032830', stockName: '삼성생명', currentPrice: 96800, changeRate: 0.52, score: 83, aiScore: 86, aiComment: '금리 안정화 수혜, PEG 극저평가', aiThemes: '저PEG,고배당,보험', originalScore: 81, rankNum: 1, reason: 'PEG 0.35, EPS성장 42%', peg: 0.35, epsGrowth: 42, roe: 12.8, per: 4.5 },
        { stockCode: '086790', stockName: '하나금융지주', currentPrice: 64300, changeRate: 0.78, score: 79, aiScore: 82, aiComment: '밸류업 + 주주환원 확대', aiThemes: '저PEG,밸류업,자사주', originalScore: 77, rankNum: 2, reason: 'PEG 0.41, EPS성장 38%', peg: 0.41, epsGrowth: 38, roe: 11.5, per: 4.2 },
        { stockCode: '034730', stockName: 'SK', currentPrice: 165000, changeRate: -0.30, score: 75, aiScore: 78, aiComment: 'AI 인프라 + SK하이닉스 지분가치', aiThemes: '지주사할인,AI인프라', originalScore: 73, rankNum: 3, reason: 'PEG 0.52, EPS성장 28%', peg: 0.52, epsGrowth: 28, roe: 9.5, per: 7.8 },
    ...
    lastUpdated: { SCALPING: new Date().toISOString(), SWING: new Date().toISOString(), TURNAROUND: new Date().toISOString(), VALUE: new Date().toISOString() }
  }
}
function getFallbackSectorData() {
  return ['반도체', '2차전지', '제약/바이오', '자동차', '금융', '철강/소재', '조선', '건설', '유틸리티', '로봇/AI'].map(name => ({
    sectorName: name, changeRate: randFloat(-3.0, 3.0), totalTradingValue: randInt(500, 5000) * 1000000000
  }))
}
function getFallbackMarketData() {
  return { kospiIndex: '2,687.45', kospiChangeRate: 0.82, kosdaqIndex: '843.21', kosdaqChangeRate: -0.35, adr: 54.5, marketStatus: '코스피 상승 · 코스닥 소폭 하락 · 시장 심리 보통' }
}
function getFallbackGlobalData() {
  return { nasdaqFutures: { price: '21,345.50', changeRate: 0.42 }, leadingSectors: [{ sectorName: '반도체/AI', changeRate: 2.1 }, { sectorName: '조선/방산', changeRate: 1.8 }] }
}
function getFallbackSmartMoney() {
  const foreign = [
    { stockCode: '005930', stockName: '삼성전자' }, { stockCode: '000660', stockName: 'SK하이닉스' },
    { stockCode: '005380', stockName: '현대차' }, { stockCode: '068270', stockName: '셀트리온' },
    { stockCode: '035420', stockName: 'NAVER' }, { stockCode: '055550', stockName: '신한지주' },
    { stockCode: '105560', stockName: 'KB금융' }, { stockCode: '003550', stockName: 'LG' },
    { stockCode: '006400', stockName: '삼성SDI' }, { stockCode: '051910', stockName: 'LG화학' }
  ].map((s, i) => ({ ...s, netBuyAmount: randInt(1000, 50000) * 1000000, rankChange: [0,1,-1,2,0,3,-2,1,-1,0][i] })).sort((a, b) => b.netBuyAmount - a.netBuyAmount)
  const institution = [
    { stockCode: '000660', stockName: 'SK하이닉스' }, { stockCode: '005930', stockName: '삼성전자' },
    { stockCode: '035420', stockName: 'NAVER' }, { stockCode: '005380', stockName: '현대차' },
    { stockCode: '032830', stockName: '삼성생명' }, { stockCode: '003670', stockName: '포스코퓨처엠' },
    { stockCode: '086790', stockName: '하나금융지주' }, { stockCode: '010120', stockName: 'LS일렉트릭' },
    { stockCode: '009150', stockName: '삼성전기' }, { stockCode: '034220', stockName: 'LG디스플레이' }
  ].map((s, i) => ({ ...s, netBuyAmount: randInt(1000, 50000) * 1000000, rankChange: [1,-1,2,0,3,-2,1,4,-1,0][i] })).sort((a, b) => b.netBuyAmount - a.netBuyAmount)
...
function getFallbackNewsData() {
  ...
  return [
    { title: '삼성전자, HBM3E 양산 본격화...엔비디아 공급 확대', summary: 'AI 반도체 수요 급증에 따라 삼성전자의 HBM 양산이 가속화되고 있다.', source: '한국경제', publishedAt: today, sentiment: '긍정' },
    { title: '코스피 2,690 돌파...반도체·조선주 강세', summary: '외국인 매수세가 유입되며 코스피가 이틀 연속 상승했다.', source: '매일경제', publishedAt: today, sentiment: '긍정' },
    { title: '미 연준 금리 동결...9월 인하 가능성 시사', summary: '제롬 파월 의장은 인플레이션 둔화 추세를 확인하며 금리 인하 가능성을 열어두었다.', source: '연합뉴스', publishedAt: today, sentiment: '긍정' },
    { title: '2차전지 소재주, 유럽 전기차 보조금 확대에 반등', summary: 'EU가 전기차 보조금을 확대하면서 관련 소재주들이 동반 상승했다.', source: '서울경제', publishedAt: today, sentiment: '긍정' },
    { title: '중국 경기 둔화 우려...제약바이오 차익 실현', summary: '중국 PMI 부진으로 아시아 증시 혼조, 바이오 업종 차익 실현 매물 출회.', source: '조선비즈', publishedAt: today, sentiment: '부정' }
  ]
}
...
export default {
  ...
  methods: {
    ...
    // Section A: AI 전략 (V2 → Java → 폴백, 3초 타임아웃)
    async loadAiStrategy() {
      try {
        ...
        // 3차: 폴백
        console.log('[Fallback] AI Strategy 더미 데이터 사용')
    ...
    // Section B: 시장 지도 (V2 → Java → 폴백, 3초 타임아웃)
    async loadMarketMap() {
      try {
    ...
    // Section C: 스마트 머니 (V2 → Java → 폴백, 3초 타임아웃)
    async loadSmartMoney() {
      try {
        ...
        const sm = getFallbackSmartMoney()
        this.tradesData.foreign = sm.foreign
        this.tradesData.institution = sm.institution
    ...
    // Section D: AI 리서치 (V2 → Java → 폴백, 3초 타임아웃)
    async loadResearch() {
      try {
