...
describe('marketFormatters — 시장 데이터 포맷터 (첫 실제 테스트)', () => {
  describe('formatNumber', () => {
    ...
    it('문자열 숫자 코어싱 (API 가 string 을 내려줘도 렌더 안 깨짐)', () =>
      expect(formatNumber('1234.5')).toBe('1,234.50'))
    it('비숫자 문자열 → "-"', () => expect(formatNumber('abc')).toBe('-'))
  })
  ...
  describe('formatChange', () => {
    ...
    it('문자열 숫자 코어싱', () => expect(formatChange('1.5')).toBe('+1.50%'))
    it('비숫자 문자열 → "-"', () => expect(formatChange('N/A')).toBe('-'))
  })
  ...
  describe('formatTradingValue', () => {
    it('결측(null/undefined) → "-" (§4c — 0원 거래로 위장 금지)', () => {
      expect(formatTradingValue(null)).toBe('-')
      expect(formatTradingValue(undefined)).toBe('-')
    })
    it('실측 0 → "0원"', () => expect(formatTradingValue(0)).toBe('0원'))
