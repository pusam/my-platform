...
관제실은 **GNB 없는 단독 라우트**라(§7 — 종목 화면이 아니라 운영 콘솔) 나가는 길을 화면이 직접
줘야 한다. 상단 `.topnav` 가 그 역할이다.

**목적지는 KPI 가 가리키는 원본 화면으로 고른다.** 관제실은 "무엇이 이상한가"를 보는 곳이고
링크는 "그럼 그건 어디서 보나"에 대한 답이다. 그 관계가 없는 화면은 넣지 않는다 —
**제2의 GNB 를 만들지 말 것**(테스트가 목적지 수를 5개로 묶어 둔다).

| 링크 | 대상 | 어느 KPI 의 원본인가 |
|---|---|---|
| ← 주식 허브 | `/stock-dashboard` | (기본 복귀) |
| 오늘 | `?tab=today` | 종합판단 후보 |
| 발굴 | `?tab=discover` | 종합판단 보드·백테스트 |
| 매매 | `?tab=trade` | 봇 게이트 · 일일손실 서킷 |
| 재무수집 | `/earnings-screener` | 재무 입력층 |

⚠ **탭 키 오타는 에러가 아니라 오작동이다.** 허브의 `mapLegacyTab` 은 모르는 키를
`'discover'` 로 흘려보내므로(`map[tab] || 'discover'`), '오늘'을 눌렀는데 발굴이 열리고
아무도 모른다. 목록은 순수 모듈 `CONTROL_ROOM_NAV` 에 두고
`ControlRoomNav.test.js` 가 **허브 매핑과 직접 대조**해 왕복을 검증한다.
...
### 4-6. 데이터 이상 점검 패널 (2026-08-27)

**"AI 시켜서 이상한 데이터 찾을 수 있나"에 대한 답이다.** 답은 "AI 는 못 찾는다, 규칙이 찾는다"이다.

크루는 **툴이 하나도 없다**(§7, 의도된 설계). DB·파일·API 어디에도 못 닿으므로
"찾아와라"를 시킬 수 없고 시켜서도 안 된다. 그래서 역할을 나눈다:

| | 하는 일 |
|---|---|
| `DataAnomalyRules` (결정적 규칙) | **탐지** — 매 스냅샷마다 판정 |
| 크루 | **우선순위·해석** — 규칙이 찾은 결과를 읽고 논한다 |

규칙 6종은 전부 **실제로 겪은 사고**에서 나왔다: 누적 판정 쏠림(8/27 96% 오판) ·
미래 날짜 행(8/26 342행) · 분기 데이터 노후 · 수집 배치 정체 ·
**배치 심박 stale**(8/28) · **주간 스냅샷 구멍**(8/28).

> **배치 심박이 왜 여기 들어왔나(2026-08-28)**: dead-man switch 판정은 이미 있었지만
> **텔레그램으로만** 갔다. 주간 리포트 경보가 4일 연속 왔는데 원인(8/23 일요일 크론 미실행)을
> 알아내려면 코드·로그·DB 를 뒤져야 했다. "알림은 오는데 화면엔 없다"가 그 반나절의 원인이다.
> 이제 관제실에서 한눈에 보이고, 크루도 컨텍스트로 받는다.

**FLAGGED 와 따로 두는 이유**: FLAGGED 는 사람이 적고 사람이 지운다. 이쪽은 코드가 매번
다시 판정하므로 "지운다"는 개념이 없고 고쳐지면 저절로 사라진다. 섞으면
"왜 안 지워지지 / 왜 사라졌지"가 헷갈린다.

**불변식**
- **정상이면 조용하다**(규칙이 null 반환). 정상에서 시끄러우면 사람이 경고를 무시하게 되고,
  그러면 다음 사고도 똑같이 지나간다 — 이 패널의 존재 이유가 무너진다.
- 각 규칙은 **"무엇을 확인하라"까지** `detail` 에 담는다. "이상함"만 알리면 다음 사람이 또 헤맨다.
- 점검 실패 = `dataAvailable:false` — "이상 없음"과 구분(§4c).
- 크루 컨텍스트에선 KPI·캘린더와 함께 **head** 에 들어가 상한 초과 시에도 안 잘린다.
- **화면은 판정을 다시 하지 않는다** — 임계는 백엔드 단일 출처.
...
| 한 턴 입력 토큰 | 프롬프트 크기 그대로 | **약 2.6만**(8KB 컨텍스트 실측. 하네스 보조 호출 포함) |
