...
- **킬스위치**: 일일 손실 -3% 도달 시 당일 매수 정지
- **스캘핑 킬스위치**: 일일 -1.5% 도달
- **연속 손절 정지**: 3회 연속 손절 시 당일 매수 정지
- **VIX 일시정지**: 글로벌 변동성 급등 시 매수 정지
- **KOSPI 하락 정지**: 지수 하락 시 정지
- **surge 신선도 가드** (phase 1): 스냅샷 15분 stale 시 매수 보류 + 텔레그램 risk
- **가격 신선도 가드** (phase 1): 매도 평가 가격 60초 stale 시 KIS 재조회. 실패 시 매도 보류
