...
## 추가 수정 (2026-08-28) — 실제 화면이 감사가 못 본 것을 찾아줬다

018260 종목상세를 실물로 열어보고 세 가지가 더 나왔다. **감사는 코드만 읽었고,
화면은 코드가 안 보여주는 것을 보여줬다.**

### 1. 지표 바의 "20일선"은 고장이 아니라 **채울 수 없는 칸**이었다 🔴

감사에서 `"20일선 대비 +3.1%" 문구 위치 미확인`이라고 남긴 것의 정체다.

```
지표 바      20일선  -                  ← 항상 이렇다
차트 해설    20일선 대비 +2.7% 이격      ← 값은 있다
```

`QuickSummaryBar.vue:93` 이 `diagnosisData.technicalAnalysis.disparity20` 을 읽는데
**`StockDiagnosisDto.TechnicalAnalysisDto` 에 그 필드가 아예 없었다.** 즉 `undefined` →
`d == null` → **어느 종목을 열어도 `-`**. 데이터 결측이 아니라 구조적 공백이다.
해설 문장은 백엔드가 별도 경로로 만들어서 값을 알고 있었고, 그래서 한 페이지가
"모른다"와 "+2.7%"를 동시에 말했다.

**수정**: `TechnicalAnalysisDto` 에 `disparity20` 추가 + `indicators.getDisparity20()` 전달.
여기서 다시 계산하지 않는다 — 재계산하면 해설과 또 갈린다.

### 2. `StockDetailService` 가 결측을 ZERO 로 채우는 곳이 **두 군데 더** 있었다 🔴

감사 때 프론트만 grep 하고 **백엔드 조립 지점을 안 봤다.** A-1/A-2 와 같은 패턴이다.

| 위치 | 코드 |
|---|---|
| `StockDetailService.java:600-602` | `.foreignNetBuy(... != null ? ... : BigDecimal.ZERO)` 3종 |
| 같은 파일 DB 요약 경로 | `BigDecimal foreignNetBuy = BigDecimal.ZERO;` 로 시작해 기여 행이 없어도 0 |

화면의 **`프로그램 +0억`** 이 그 증거였다. 어제 프론트를 `netBuyText`(null→'-')로 고쳤는데
백엔드가 여전히 0 을 보내서 안 걸렸다.

**수정**: ZERO 폴백 제거 + 기여 행 추적(`foreignHasRow`/`instHasRow`/`programHasValue`).
실측 0(상위20 진입 + 매수·매도 동일)은 그대로 0 으로 나간다.

> 같은 파일이 **체결강도는 이미 제대로** 하고 있었다 —
> `// 체결강도 없으면 null 유지 — 100(균형) 위장 금지`.
> 그 원칙이 순매수 3종에만 적용되지 않았다.

**소비자 안전 확인**: `getForeignNetBuy()` 등을 쓰는 곳 8군데가 **전부 이미 null 가드**를
갖고 있었다(`!= null`, 삼항, `== null ||`). null 을 내보내도 NPE 가 없다.

### 3. B-4(같은 페이지 두 값)가 실물로 확인됐다

```
지표 바        외국인 순매수 +211억     ← 상위20 진입일 최대 5일 합
투자자별 수급   외국인 +20억            ← 당일
매수 근거      "금일 외국인 순매수 +20억"
```

**11배 차이**인데 라벨은 둘 다 "외국인"이다. 라벨 문구는 여전히 사람 판단으로 남긴다.
"
