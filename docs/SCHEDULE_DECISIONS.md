...
| 2026-08-27 | 실적 분기소스 전환 (R1) | 판정보류(표본부족) | 입력 복구 완료(커버리지 2,289 · 누적판정 95→1,763 · TTM 2.09배 부풀림 해소). 그러나 변별력 미달 — 분기경로 scoring 817/2,289 = **35.7%**. 임계 스윕 20/30/50/100/200 → 35.7/32.6/27.7/21.5/16.6%. **TURNAROUND 226건(9.9%)이 임계와 무관한 바닥**이라 임계 조정만으론 10%대 불가(임계 200에서도 379건 중 226건이 turnaround) | **TURNAROUND 규칙 재설계 선행**(아래 신규 안건). 그때까지 플래그 OFF 유지. 재판정일 2026-09-16 |
...
calendar:
  ...
  - id: earnings-quarterly-switch
    ...
    trigger: "선행 조건 2026-08-27 충족(coverage.distinctStocks 2,289 ≥ 2000). 남은 것은 사람 판단 — compare=true 로 레거시/분기 건수 비교 후 켜고, 켠 날짜를 아래 표에 기록"
    result: 2026-08-26 파이프라인 배포(V55 + 두 경로 분리), 플래그 OFF. 8/26 오후 prod 실측으로 R1 진단이 두 번 틀렸음이 확인됐고(死가 아니라 커버리지 ~90종목), 8/27 진짜 원인 확정 — KIS 재무 API 3종의 tr_id 가 서로 어긋나 경로와 다른 리포트를 받아오고 있었다(경로가 아니라 tr_id 로 디스패치, 에러 없이 조용히). 정정 배포 후 08:30 배치에서 복구 확인됨 — 유니버스 2,656 · 스키마 경고 0 · 분기 원본 2,618종목 · coverage 2,289 · maxPeriodEnd 2026-06-30. 켤 조건 충족. 남은 건 사람이 켜고 날짜를 기록하는 것뿐이다.
  - id: turnaround-rule-redesign
    title: TURNAROUND 규칙 재설계 (QoQ 적자→흑자)
    due: 2026-09-16
    status: pending
    trigger: "산식 변경이라 날짜가 아니라 사람 판단이 트리거. 재수집 안정화(누적판정 2,000+ 유지) 확인 후 착수"
    result: 2026-08-27 실측 TURNAROUND 226건(9.9%). 2026-08-28 연속성 분해 결과 209건 중 consecutiveLoss 98 / singleQuarterBlip 111 / undecidable 0 — 절반 이상이 '한 분기만 적자'였다. 후보안 (나) 채택하여 분기 경로에 연속 적자 조건 적용(classifyQuarterly). ⚠ TURNAROUND 만 막으면 효과가 없다 — prevOp<0 이면 변화율 분모가 적자 규모라 POSITIVE 경로에서 똑같이 20점을 받는다. 그래서 연속성 미달은 변화율로 재분류하지 않고 서프라이즈에서 제외한다. 임계(±20%)는 건드리지 않았다 — 어느 값이 옳은지는 forward 성과 측정이 필요하고 근거 없이 올리면 상수가 하나 더 생긴다. 레거시(일별) 경로는 3분기를 못 구해 무변경이며, 이 변경은 플래그를 켤 때만 효과가 있다.
```
