...
calendar:
  ...
  - id: turnaround-rule-redesign
    ...
    result: 2026-08-27 실측 — 분기 입력 복구 후 TURNAROUND 가 226건(커버리지 2,289 의 9.9%)으로 나왔고 임계와 무관해 earnings 변별력의 바닥을 만든다. 현재 규칙은 prevOp<0 && latestOp>0 을 QoQ 로 판정해 최고점 20점을 준다 — 변동성 큰 중소형주가 분기마다 오가는 것을 턴어라운드로 잡는다. 후보안 (가) 연간/YoY 기준으로 전환 (나) 흑자 규모·연속성 하한 추가 (다) 점수를 20에서 낮춤. 어느 쪽도 composite 점수를 바꾸므로 실측 분포를 보고 정할 것.
```
