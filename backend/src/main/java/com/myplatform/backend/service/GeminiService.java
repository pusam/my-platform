...
public class GeminiService {
    ...
    public String analyzeStockDetail(String stockDataSummary) {
        ...
        String prompt = String.format("""
                ...
                1. 객관적 분리 원칙:
                   - "수급 데이터"(외인/기관 매매량, 프로그램 매매)와 "뉴스/이벤트 데이터"는 반드시 독립적으로 평가하라.
                   - 수급이 부정적이더라도 그것을 뉴스의 해석에 영향을 주지 마라.
                   - 뉴스가 긍정적이더라도 그것을 수급 해석에 영향을 주지 마라.

                2. 뉴스 팩트체크 원칙:
                   - 뉴스에 "투자 유치", "신사업 확대", "수주", "매출 성장", "흑자 전환" 등 호재성 키워드가 있다면, 이를 매도 근거나 리스크 요인에 넣지 마라.
                   - 호재 뉴스는 매수 근거 또는 긍정 요인으로만 분류하라.
                   - 악재 뉴스(적자, 소송, 규제, 감사의견 거절 등)만 리스크 요인에 넣어라.

                3. 논리적 모순(Hallucination) 방지:
                   - "투자 유치 → 불확실성 증가", "신사업 확대 → 리스크", "매출 증가 → 매도 근거" 같은 호재를 악재로 뒤집는 논리적 모순을 절대 생성하지 마라.
                   - 분석 결과를 작성한 뒤, 호재성 단어가 리스크/매도 근거에 포함되어 있는지 스스로 검증하라.
                   - 수급 데이터(매도)와 뉴스 데이터(호재)가 상충할 경우, 뉴스를 억지로 악재로 해석하지 마라.
                     "기관 매도세로 수급은 불안하나, 매출 증가 등 펀더멘털 뉴스는 긍정적임"과 같이 객관적으로 분리하여 균형 잡힌 코멘트를 작성하라.
                   - 절대 금지 패턴: 기관/외인이 매도 중이라는 이유만으로 호재 뉴스(매출 증가, 합병, 투자 유치, 수주 등)를 매도 근거에 넣는 행위.

                4. 긍정 신호 해석 원칙:
                   - "신고가 돌파", "52주 신고가", "코스피/코스닥 신고가" 등은 강한 매수 신호로 해석하라.
                   - 외인/기관 대량 순매수는 긍정적 수급 신호로 해석하라.
                   - 단, 단기 급등 시 과열 여부(RSI, 괴리율)만 리스크 요인으로 언급 가능하다.
                   - "신고가 돌파 → 위험", "외인 매수 유입 → 부정적" 같은 해석은 절대 금지.

                5. 자회사/계열사 뉴스 연동:
                   - 뉴스 중 자회사/계열사/모회사 관련 내용이 있으면, "자회사(또는 계열사) OO의 호재(또는 악재) 연동"이라고 명시하라.
                   - 예: SK스퀘어 분석 시 SK하이닉스 뉴스가 있으면 → "자회사 SK하이닉스의 실적 호조가 지주사에 긍정적 영향"

                6. 비즈니스 확장 뉴스 해석 원칙 (★ 최우선 적용):
                   - 수주전 참여, 신사업 진출, 대규모 투자 유치, 해외 수출 계약, 재건축/재개발 수주, 신규 플랜트 수주 등 기업의 '비즈니스 확장' 관련 뉴스는 수급이 안 좋더라도 억지로 '실적 악화'나 '위험 요인'으로 해석하지 마라.
                   - 이러한 뉴스는 명백한 '호재성 모멘텀'으로 분류하고, "단기 수급은 부진하나 중장기 수주 모멘텀은 존재함"과 같이 매우 객관적이고 균형 잡힌 시각으로 분석글을 작성하라.
                   - 절대 금지 예시: "GS건설 압구정 재건축 수주전 참여 → 실적 악화 가능성" (이런 해석은 분석 무효)
                   - 올바른 예시: "GS건설 압구정 재건축 수주전 참여 → 대형 수주 모멘텀, 수주 확정 시 실적 개선 기대"

                7. 시장 지수 상승 뉴스 해석 원칙 (★ 최우선 적용):
                   - "코스피 역대 최고가 돌파", "코스피 신고가", "코스닥 사상 최고", "지수 강세", "증시 랠리" 등 시장 전체의 상승 뉴스는 해당 종목에 긍정적 시장 환경으로 해석하라.
                   - 이를 "과열 우려", "고점 매도 신호", "거품 경고", "리스크 경고" 등 매도 근거나 부정적 리스크로 절대 해석하지 마라.
                   - 시장 상승은 개별 종목에게 "우호적 시장 환경", "투자 심리 개선", "유동성 유입" 등 긍정 요인으로만 분류하라.
                   - 절대 금지 예시: "코스피 역대 최고가 돌파 → 고점 리스크, 매도 권고" (이런 해석은 분석 무효)
                   - 올바른 예시: "코스피 신고가 랠리가 해당 종목에도 우호적 시장 환경을 제공하고 있음"
                   - 단, 해당 종목 자체의 과열 지표(RSI 80 이상 등)만 리스크로 언급 가능. 시장 지수 상승 자체를 리스크로 연결하는 것은 금지.
                ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                ...
                ■ 뉴스/이벤트 해석: (뉴스 데이터만 기반으로 작성)
                - 최근 뉴스의 성격 (호재/악재/중립) 판단
                - 주가에 미치는 영향

                ■ 리스크 요인: (실제 악재만 기재, 호재를 리스크로 변환 절대 금지)
                - 주의해야 할 핵심 위험 (2-3가지)
                - 수급 매도세는 여기에 기재 가능하나, 호재 뉴스(매출 증가, 합병, 수주 등)는 절대 여기에 넣지 마라
