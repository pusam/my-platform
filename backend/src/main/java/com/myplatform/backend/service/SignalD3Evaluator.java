...
public final class SignalD3Evaluator {
     ...
     * 비율이 정상 범위 밖으로 확실히 나가는 재조정만 걸린다. <b>2:1 도 놓칠 수 있다</b>: 정상 변동 1.2 ×
     * 분할 조정 0.5 = 0.6 은 하한 0.53 위라 통과한다(반례를 {@code SignalD3EvaluatorTest} 가 고정).
     * 3:2·5:4 처럼 비율이 1 에 가까운 재조정은 더더욱 못 잡는다. 작은 비율의 재조정은 별도 이벤트
    ...
    public enum Status {
        ...
        NO_INDEX,
        /**
         * 봉을 받지 못했다(빈 응답·수집 예외·저장 실패·동시 수집 중) — 배치가 붙이는 상태. 사유는 note,
         * 시각은 d3_evaluated_at. 일시 장애일 수 있어 다음 날 다시 시도한다(다른 재시도보다 짧게).
         */
        FETCH_FAILED
    }
