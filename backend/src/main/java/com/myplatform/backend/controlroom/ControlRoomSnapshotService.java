...
public class ControlRoomSnapshotService {
    ...
    private ControlRoomSnapshotDto.Anomalies anomalies(LocalDate today) {
        ...
        try {
            ...
            // 재료 분류 파이프라인 정지 — 2026-07-01 "7일 연속 100% NONE" 재발 감지.
            found.add(DataAnomalyRules.catalystStall(
                    catalystRepository.findDailyStatsSince(today.minusDays(10)).stream()
                            .map(d -> new DataAnomalyRules.CatalystDayStat(
    ...
    /**
     * 수익 수치의 출처(2026-09-21 전환) — 게이트는 <b>V59 교정 평가</b>(기록 시점 가격 → 달력이 정한 D+3 의
     * KRX 종가, 창은 D+1~D+3 세 봉)를 읽는다. 2026-09-17~20 은 기존 3일 평가(배치 실행 시점 현재가, 창 상한
     * 없음)를 읽었고 그 구값은 컬럼에 남아 있지만 여기엔 섞이지 않는다. 교정 평가가 OK 가 아닌 행(봉 결측·
     * 거래정지·지수 없음)은 표본에서 빠지므로 행 수가 구 기준보다 적을 수 있다 — 전환일이 표본 경계다.
     */
    static final String TRUST_GATE_MEASUREMENT_CAVEAT =
            "수익·낙폭은 V59 교정 평가(D+3 KRX 종가) 기준, 2026-09-21 전환 — 교정 미평가 행은 표본에서 빠진다. ";

    /**
     * 종합판단 보드(momentum) 후보를 등급별로 센다. 보드 조회 실패는 0 이 아니라 데이터 없음이다.
     *
     * <p><b>0 건일 때 이유를 함께 싣는다.</b> 2026-09-16 전엔 {@code JudgmentBoardService.getBoard} 가
     * 후보 조회 실패도 예외를 삼키고 빈 목록을 돌려줘서 "진짜 0건"·"조회 실패"·"입력 노후로 미채점"이
     * 전부 같은 0 이었다(2026-08-24 실발생 — 서버 4일 다운 뒤 노후 가드가 채점을 거부해 0 이 됐는데
     * 화면엔 "0종목"만 떴다). 지금은 보드가 조회 실패를 예외로 올려 아래 catch 가 {@code dataAvailable=false}
     * 로 받으므로, 이 블록의 0 은 "컷 통과 0건" 아니면 "입력 노후로 미채점"이다 — 그 둘은
     * <b>추천 스냅샷 신선도</b>로 가른다.
     */
    ...
    static String emptyReason(boolean empty, LocalDateTime latestSnapshotAt, Boolean stale) {
        ...
        return "후보 0건 — 보드가 빈 결과를 반환했다. 조회 실패는 보드가 예외로 올리므로(2026-09-16) "
                + "여기 0건은 '컷 통과 0건'이다.";
    }
